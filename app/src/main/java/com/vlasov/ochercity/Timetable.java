...
import com.vlasov.helpers.Cache;
...
import static com.vlasov.ochercity.Functions.urlEncodeUTF8;
...
public class Timetable {
    ...
    interface onDownloadedListener {
        void onDownloaded(Timetable item);
    }

    interface onMultipleDownloadedListener {
        void onDownloaded(List<Timetable> items);
    }

    public Timetable(int id, String name, String[] imageUrl) {
        this.id = id;
        this.name = name;
     ...
     * @param JSONItem
     */
    public Timetable(JSONObject JSONItem) {
        try {
     ...
     */
    public static void loadMultiple(FragmentActivity activity, int offset, int limit, String search, final Timetable.onMultipleDownloadedListener downloadedListener) {
     ...
     * @param downloadedListener
     */
    public static void loadMultiple(final FragmentActivity activity, int offset, int limit, String search, String category, final Timetable.onMultipleDownloadedListener downloadedListener) {
     ...
     * @param downloadedListener
     */
    public static void loadMultiple(final FragmentActivity activity, final int[] ids, final Timetable.onMultipleDownloadedListener downloadedListener) {
     ...
     * @param downloadedListener
     */
    public static void loadMultiple(final FragmentActivity activity, int offset, int limit, Map<String, String> params, boolean top, final Timetable.onMultipleDownloadedListener downloadedListener) {

        // Instantiate the RequestQueue.
        RequestQueue queue = Volley.newRequestQueue(activity);
        final String url = Configurations.SERVER_URL + "api/timetables/" + ((top) ? ("top/viewed/") : ("")) + +offset + "/" + limit + "?" + urlEncodeUTF8(params);
        Log.e("URL", url);
     ...
     * @param downloadedListener
     */
    public static void decodeMultiple(String responseStr, final Timetable.onMultipleDownloadedListener downloadedListener) {
        try {
            ...
            for (int i = 0; i < response.length(); i++) {
                ...
                timetable.add(new Timetable(jsonObject));
            }
            downloadedListener.onDownloaded(timetable);
        }
     ...
     * @param downloadedListener
     */
    public static void loadSingle(final FragmentActivity activity, int id, final Timetable.onDownloadedListener downloadedListener) {

        // Instantiate the RequestQueue.
        RequestQueue queue = Volley.newRequestQueue(activity);
        final String url = Configurations.SERVER_URL + "api/timetable/" + id;
        final Cache cache = new Cache(activity);
        Log.e("URL", url);
     ...
     * @param downloadedListener
     */
    public static void decodeSingle(final FragmentActivity activity, String responseStr, final Timetable.onDownloadedListener downloadedListener) {
        try {
            JSONObject response = new JSONObject(responseStr);
            System.out.println(response.toString(2));
            final Timetable timetable = new Timetable(response);
            downloadedListener.onDownloaded(timetable);
        }
        // Try and catch are included to handle any errors due to JSON
        catch (JSONException e) {
...
//    }
    ...
    public static void send(Context context, String data) {
        if (context != null) {
            ...
            String url = Configurations.SERVER_URL + "api/timetable/" + data;
...
//        }
        ...
        if (savedInstanceState == null) {
            ...
            if (extras != null) {
                return extras.getInt(SingleTimetableActivity.ITEM_KEY, -10);
            }
        } else {
            if (savedInstanceState.containsKey(SingleTimetableActivity.ITEM_KEY))
                return (int) savedInstanceState.getSerializable(SingleTimetableActivity.ITEM_KEY);
        }
